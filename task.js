...
const maxAllPostCount = 22 * 1000;
const minNewPostCount = 280;
const maxNewPostCount = 700;
...
function fixMarkdownContent(content) {
  ...
  content = lines.map((line) => {
          ...
          // ](**)the => ](**) the
          line = line.replace(/\]\s*\(\s*(.*?)\s*\)(\w)/g, ']($1) $2')
  ...
  const updateDateWithContent = (content) => {
    try {
      ...
      for (let i = 0; i < lines.length; ++i) {
        const line = lines[i];
        if (beginTagIndex < 0 && line.startsWith("---")) {
           beginTagIndex = i;
           continue;
        }
      ...
      // 在 beginTagIndex 和 endTagIndex 之间查找 date 和 updated
      for (let i = beginTagIndex; i < endTagIndex; ++i) {
        const line = lines[i];
        if (line.startsWith("date:")) {
          lines.splice(i, 1, `date: ${createDate.toISOString()}`);
        }

        if (line.startsWith("updated:")) {
          lines.splice(i, 1, `updated: ${updateDate.toISOString()}`);
        }
      }
...
<ins class="adsbygoogle"
  ...
  if (posts.length < maxAllPostCount) {
    ...
    if (!fs.existsSync(sourceDir)) {
      ...
      // 如果整体文章总数超过 60%，采用1/10的数量更新
      const _minCount = posts.length >= maxAllPostCount*0.6 ? minNewPostCount*0.1 : minNewPostCount;
      const _maxCount = posts.length >= maxAllPostCount*0.6 ? maxNewPostCount*0.1 : maxNewPostCount;

      let new_files = new_posts.slice(0, randomIntFromInterval(_minCount, _maxCount));
...
const backupGit = () => {
  ...
  try {
    ...
    try {
      ...
      [
        "themes/redefine/package-lock.json",
        "package-lock.json",
        ".backup-date",
        "yarn.lock",
        "submit.google.js"
        ...
        try {
          execSync(`rm -fr ./${file}`, { cwd: __dirname, maxBuffer: 100 * 1024 * 1024, });
          execSync(`git add ${file}`, { cwd: __dirname, maxBuffer: 100 * 1024 * 1024, });
        } catch (e){}
      })

      // 这些文件不应该提交
      const keyNodeJsFileList = [
        "create-last-publish-date.js",
        "remove-duplicated-lines.js",
        "try-deploy.js",
        "task.js"
      ];
      keyNodeJsFileList.forEach((file) => {
        try {
          exec(`git rm --cached ${file}`, { cwd: __dirname });
        } catch (e) {}
      });
    } catch (e) {}
    ...
    try {
      ...
      ['source/', 'themes/', '_config.redefine.yml', '_config.yml', 'package.json', 'readme.md', 'init.js', 'submit.bing.js', ".gitattributes"].forEach(file => {
        try {
          // 需要备份的关键文件
          execSync(`git add ${file}`, { cwd: __dirname, maxBuffer: 100 * 1024 * 1024, });
        } catch (e){}
    ...
    const tryPushToGitHub = (args) => {
      ...
      while (tryTimes > 0) {
        tryTimes = tryTimes - 1;
        console.log(`${tryTimes}.执行: git ${args.join(" ")}`);
        try {
    ...
    if (!isInstallGitLFS) {
      try {
         tryPushToGitHub([`push`, `--set-upstream`, `origin`, `main`]);
      } catch (e) {
         tryPushToGitHub([`push`, `--force`, `--set-upstream`, `origin`, `main`]);
      }
    }
